...
vec3 addScanlines(vec3 color, vec2 uv) {
    float scan = sin(uv.y * 1200.0) * 0.04;
    float glow = 0.02 * sin(uv.y * 80.0);
    return color + scan + glow;
}

vec3 addEdgeGlow(vec3 color, vec2 uv) {
    float d = distance(uv, vec2(0.5));
    float glow = smoothstep(0.45, 0.5, d);
    return color + vec3(glow * 1.2, glow * 0.45, glow * 0.15);
...
void main() {
    ...
    int cx = itemIndex % cellsPerRow;
    int cy = itemIndex / cellsPerRow;
    
    vec2 cell = vec2(1.0 / float(cellsPerRow));
    vec2 offset = vec2(float(cx), float(cy)) * cell;

    vec2 uv = vec2(vUvs.x, 1.0 - vUvs.y);
    uv = uv * cell + offset;

    vec4 texColor = texture(uTex, uv);
    vec3 color = texColor.rgb;

    color = addScanlines(color, uv);
    color = addEdgeGlow(color, vUvs);

    outColor = vec4(color, texColor.a * vAlpha);
}
`;
...
class InfiniteGridMenu {
  ...
  public preventAutoSnapUntil = 0;

  constructor(
    private canvas: HTMLCanvasElement,
    private items: RawMenuItem[],
  ...
  // Mapeo manual de ítems → vértices reales en la esfera
public itemIndexMap: Record<string, number> = {
  ranked: 12,
  create: 27,
  join: 33,
  friends: 48,
  ...
  // --- Permite saltar directamente a un ítem desde afuera ---
public jumpToIndex(index: number) {
  if (index < 0 || index >= this.instancePositions.length) return;

  const target = this.instancePositions[index];
  const dir = vec3.normalize(vec3.create(), target);

  // 🔥 Evita que autosnap anule tu salto
  this.preventAutoSnapUntil = performance.now() + 1000; // 1 segundo

  // Rotación hacia el objetivo
  this.control.snapTargetDirection = dir;

  // Pequeño empujón para iniciar animación
  this.control.rotationVelocity = 0.8;
  ...
  const gl = this.canvas.getContext('webgl2', {
    antialias: true,
    alpha: true,          // ⬅️ AHORA el canvas respeta el alpha
  ...
  // ⛔ Durante el salto NO permitir el auto-snap
  if (performance.now() < this.preventAutoSnapUntil) {
    return;
  }
...
const defaultItems: RawMenuItem[] = [
  {
...
window.addEventListener("jumpToItem", handleJump);
  ...
  return (
    <div className="relative w-full h-full">
      <TopMenuBar
  items={[
    { id: "ranked", label: t("ranked") },
    { id: "create", label: t("create") },
    { id: "join", label: t("join") },
    { id: "friends", label: t("2vs2") },
  ]}
  activeId={activeItem?.link ?? null}
  onSelect={(id) => {
...
/>
  ...
  {/* 🔥 animate glow behind the circle */}
  <div
    className="
      absolute w-[520px] h-[520px]
      max-md:w-[330px] max-md:h-[330px]
      rounded-full
      bg-black
      blur-[90px]
  ...
  />
      ...
      {activeItem && (
        <>
         <h2
  className={`
    absolute font-black tracking-tight drop-shadow-[0_0_10px_#802500]
    transition-all duration-500 text-center leading-tight
    
    /* Desktop */
    left-[3.4em] top-1/2 -translate-y-1/2 translate-x-[20%] text-[4rem] text-orange-700

    /* Mobile */
    max-md:left-1/2 max-md:top-[29%] max-md:-translate-x-1/2
    max-md:text-[2.4rem] max-md:w-[90%] max-md:text-orange-300

    ${isMoving ? "opacity-0" : "opacity-100"}
  `}
>
  {(() => {
    const [first, ...rest] = activeItem.title.split(" ");
    return (
      <>
        {first}
        <br />
  ...
  className={`
    ...
    /* Desktop */
    right-[11%] top-[42%] -translate-y-1/2 max-w-[25ch]

    /* Mobile */
    max-md:left-1/2 max-md:top-[65%] max-md:-translate-x-1/2
    max-md:text-[1.1rem] max-md:w-[80%] max-md:max-w-[30ch]
...
>
  ...
  className={`
    ...
    grid place-items-center font-bold tracking-wide
    bg-gradient-to-br from-[#ff7a18] to-[#c2410c]
    shadow-[0_0_25px_#ff7a1899] border border-[#ff7a18]
    rounded-xl transition-all duration-300
    
    /* Desktop */
    w-[200px] h-[60px] bottom-[7em] 

    /* Mobile */
    max-md:w-[160px] max-md:h-[50px]
    max-md:bottom-[15%]
  ...
  `}
>
  {activeItem?.link === "ranked" && t("play")}
  {activeItem?.link === "create" && t("create_room")}
  {activeItem?.link === "join" && t("join_room")}
  {activeItem?.link === "friends" && t("coming_soon")}
</div>
