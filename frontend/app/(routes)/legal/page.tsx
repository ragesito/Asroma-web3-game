...
export default function LegalPage() {
  ...
  return (
    <main>
      ...
      <div
        className="min-h-screen h-auto bg-cover bg-center relative text-white"
        style={{
          backgroundImage:
      ...
      >
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-4">
          {/* Tabs */}
          <div className="flex justify-center gap-6 mb-4 md:mb-7 text-sm">
            <button
              className={tab === "terms" ? "text-white" : "text-white/50"}
              onClick={() => window.history.pushState(null, "", "/legal?tab=terms")}
            >
              Terms of Service
            </button>
            <button
              className={tab === "privacy" ? "text-white" : "text-white/50"}
              onClick={() => window.history.pushState(null, "", "/legal?tab=privacy")}
            >
              Privacy Policy
            </button>
          </div>

          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-8 space-y-5 text-white/90 text-sm leading-relaxed max-h-[62vh] overflow-y-auto">
